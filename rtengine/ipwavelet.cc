...
//  if(settings->verbose) printf("Wav mul 0=%f 1=%f 2=%f 3=%f 4=%f 5=%f 6=%f 7=%f 8=%f 9=%f\n",cp.mul[0],cp.mul[1],cp.mul[2],cp.mul[3],cp.mul[4],cp.mul[5],cp.mul[6],cp.mul[7],cp.mul[8],cp.mul[9]);
    ...
    cp.b_lsty = static_cast<float> (params->wavelet.hueskinsty.getBottomLeft()) / 100.0f;
    cp.t_lsty = static_cast<float> (params->wavelet.hueskinsty.getTopLeft()) / 100.0f;
    cp.b_rsty = static_cast<float> (params->wavelet.hueskinsty.getBottomRight()) / 100.0f;
    cp.t_rsty = static_cast<float> (params->wavelet.hueskinsty.getTopRight()) / 100.0f;
