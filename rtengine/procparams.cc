...
// save C/A correction
        ...
        //save Local rgb
        if (!pedited || pedited->localwb.enabled) {
            keyFile.set_boolean ("Locrgb", "Enabled", localwb.enabled);
        }


        if (!pedited || pedited->localwb.locY) {
            keyFile.set_integer ("Locrgb", "LocY", localwb.locY);
        }

        if (!pedited || pedited->localwb.locX) {
            keyFile.set_integer ("Locrgb", "LocX", localwb.locX);
        }

        if (!pedited || pedited->localwb.locYT) {
            keyFile.set_integer ("Locrgb", "LocYT", localwb.locYT);
        }

        if (!pedited || pedited->localwb.locXL) {
            keyFile.set_integer ("Locrgb", "LocXL", localwb.locXL);
        }

        if (!pedited || pedited->localwb.centerX) {
            keyFile.set_integer ("Locrgb", "CenterX", localwb.centerX);
        }

        if (!pedited || pedited->localwb.centerY) {
            keyFile.set_integer ("Locrgb", "CenterY", localwb.centerY);
        }

        if (!pedited || pedited->localwb.circrad) {
            keyFile.set_integer ("Locrgb", "Circrad", localwb.circrad);
        }

        if (!pedited || pedited->localwb.thres) {
            keyFile.set_integer ("Locrgb", "Thres", localwb.thres);
        }

        if (!pedited || pedited->localwb.proxi) {
            keyFile.set_integer ("Locrgb", "Proxi", localwb.proxi);
        }

        if (!pedited || pedited->localwb.Smethod) {
            keyFile.set_string ("Locrgb", "Smethod", localwb.Smethod);
        }

        if (!pedited || pedited->localwb.nbspot) {
            keyFile.set_integer ("Locrgb", "Nbspot", localwb.nbspot);
        }

        if (!pedited || pedited->localwb.anbspot) {
            keyFile.set_integer ("Locrgb", "ANbspot", localwb.anbspot);
        }

        if (!pedited || pedited->localwb.retrab) {
            keyFile.set_integer ("Locrgb", "Retrab", localwb.retrab);
        }

        if (!pedited || pedited->localwb.hueref) {
            keyFile.set_double ("Locrgb", "Hueref", localwb.hueref);
        }

        if (!pedited || pedited->localwb.chromaref) {
            keyFile.set_double ("Locrgb", "Chromaref", localwb.chromaref);
        }

        if (!pedited || pedited->localwb.lumaref) {
            keyFile.set_double ("Locrgb", "Lumaref", localwb.lumaref);
        }

        if (!pedited || pedited->localwb.transit) {
            keyFile.set_integer ("Locrgb", "Transit", localwb.transit);
        }

        if (!pedited || pedited->localwb.sensi) {
            keyFile.set_integer ("Locrgb", "Sensi", localwb.sensi);
        }

        if (!pedited || pedited->localwb.qualityMethod) {
            keyFile.set_string ("Locrgb", "qualityMethod", localwb.qualityMethod);
        }

        if (!pedited || pedited->localwb.wbMethod) {
            keyFile.set_string ("Locrgb", "wbMethod", localwb.wbMethod);
        }

        if (!pedited || pedited->localwb.wbcamMethod) {
            keyFile.set_string ("Locrgb", "WbcamMethod", localwb.wbcamMethod);
        }

        if (!pedited || pedited->localwb.temp) {
            keyFile.set_double ("Locrgb", "Temp", localwb.temp);
        }

        if (!pedited || pedited->localwb.green) {
            keyFile.set_double ("Locrgb", "Green", localwb.green);
        }

        if (!pedited || pedited->localwb.equal) {
            keyFile.set_double ("Locrgb", "Equal", localwb.equal);
        }

        if (!pedited || pedited->localwb.expwb) {
            keyFile.set_boolean ("Locrgb", "Expwb", localwb.expwb);
        }

        if (!pedited || pedited->localwb.gamma) {
            keyFile.set_boolean ("Locrgb", "Gamma", localwb.gamma);
        }
...
// load black & white
        ...
        if (keyFile.has_group ("Locrgb")) {
            if (keyFile.has_key ("Locrgb", "Enabled"))  {
                localwb.enabled  = keyFile.get_boolean ("Locrgb", "Enabled");

                if (pedited) {
                    pedited->localwb.enabled = true;
                }
            }



            if (keyFile.has_key ("Locrgb", "Smethod"))  {
                localwb.Smethod  = keyFile.get_string ("Locrgb", "Smethod");

                if (pedited) {
                    pedited->localwb.Smethod = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "LocY"))  {
                localwb.locY  = keyFile.get_integer ("Locrgb", "LocY");

                if (pedited) {
                    pedited->localwb.locY = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "LocX")) {
                localwb.locX = keyFile.get_integer  ("Locrgb", "LocX");

                if (pedited) {
                    pedited->localwb.locX = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "LocYT"))  {
                localwb.locYT  = keyFile.get_integer ("Locrgb", "LocYT");

                if (pedited) {
                    pedited->localwb.locYT = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "LocXL")) {
                localwb.locXL = keyFile.get_integer  ("Locrgb", "LocXL");

                if (pedited) {
                    pedited->localwb.locXL = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "CenterX"))  {
                localwb.centerX  = keyFile.get_integer ("Locrgb", "CenterX");

                if (pedited) {
                    pedited->localwb.centerX = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "CenterY"))  {
                localwb.centerY  = keyFile.get_integer ("Locrgb", "CenterY");

                if (pedited) {
                    pedited->localwb.centerY = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "Circrad"))  {
                localwb.circrad  = keyFile.get_integer ("Locrgb", "Circrad");

                if (pedited) {
                    pedited->localwb.circrad = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "Thres"))  {
                localwb.thres  = keyFile.get_integer ("Locrgb", "Thres");

                if (pedited) {
                    pedited->localwb.thres = true;
                }
            }


            if (keyFile.has_key ("Locrgb", "Proxi"))  {
                localwb.proxi  = keyFile.get_integer ("Locrgb", "Proxi");

                if (pedited) {
                    pedited->localwb.proxi = true;
                }
            }



            if (keyFile.has_key ("Locrgb", "Sensi"))  {
                localwb.sensi  = keyFile.get_integer ("Locrgb", "Sensi");

                if (pedited) {
                    pedited->localwb.sensi = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "Nbspot"))  {
                localwb.nbspot  = keyFile.get_integer ("Locrgb", "Nbspot");

                if (pedited) {
                    pedited->localwb.nbspot = true;
                }
            }



            if (keyFile.has_key ("Locrgb", "ANbspot"))  {
                localwb.anbspot  = keyFile.get_integer ("Locrgb", "ANbspot");

                if (pedited) {
                    pedited->localwb.anbspot = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "Retrab"))  {
                localwb.retrab = keyFile.get_integer ("Locrgb", "Retrab");

                if (pedited) {
                    pedited->localwb.retrab = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "Hueref"))  {
                localwb.hueref  = keyFile.get_double ("Locrgb", "Hueref");

                if (pedited) {
                    pedited->localwb.hueref = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "Chromaref"))  {
                localwb.chromaref  = keyFile.get_double ("Locrgb", "Chromaref");

                if (pedited) {
                    pedited->localwb.chromaref = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "Lumaref"))  {
                localwb.lumaref  = keyFile.get_double ("Locrgb", "Lumaref");

                if (pedited) {
                    pedited->localwb.lumaref = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "Transit"))  {
                localwb.transit  = keyFile.get_integer ("Locrgb", "Transit");

                if (pedited) {
                    pedited->localwb.transit = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "qualityMethod"))  {
                localwb.qualityMethod  = keyFile.get_string ("Locrgb", "qualityMethod");

                if (pedited) {
                    pedited->localwb.qualityMethod = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "wbMethod"))  {
                localwb.wbMethod  = keyFile.get_string ("Locrgb", "wbMethod");

                if (pedited) {
                    pedited->localwb.wbMethod = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "WbcamMethod"))  {
                localwb.wbcamMethod  = keyFile.get_string ("Locrgb", "WbcamMethod");

                if (pedited) {
                    pedited->localwb.wbcamMethod = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "Expwb")) {
                localwb.expwb = keyFile.get_boolean ("Locrgb", "Expwb");

                if (pedited) {
                    pedited->localwb.expwb = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "Temp"))  {
                localwb.temp  = keyFile.get_double ("Locrgb", "Temp");

                if (pedited) {
                    pedited->localwb.temp = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "Green"))  {
                localwb.green  = keyFile.get_double ("Locrgb", "Green");

                if (pedited) {
                    pedited->localwb.green = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "Equal"))  {
                localwb.equal  = keyFile.get_double ("Locrgb", "Equal");

                if (pedited) {
                    pedited->localwb.equal = true;
                }
            }

            if (keyFile.has_key ("Locrgb", "Gamma"))  {
                localwb.gamma  = keyFile.get_boolean ("Locrgb", "Gamma");

                if (pedited) {
                    pedited->localwb.gamma = true;
                }
            }
...
//      && dirpyrDenoise.perform == other.dirpyrDenoise.perform
        ...
        && localwb.enabled == other.localwb.enabled
        && localwb.Smethod == other.localwb.Smethod
        && localwb.locY == other.localwb.locY
        && localwb.locX == other.localwb.locX
        && localwb.locYT == other.localwb.locYT
        && localwb.locXL == other.localwb.locXL
        && localwb.centerX == other.localwb.centerX
        && localwb.centerY == other.localwb.centerY
        && localwb.circrad == other.localwb.circrad
        && localwb.thres == other.localwb.thres
        && localwb.proxi == other.localwb.proxi
        && localwb.sensi == other.localwb.sensi
        && localwb.transit == other.localwb.transit
        && localwb.qualityMethod == other.localwb.qualityMethod
        && localwb.wbMethod == other.localwb.wbMethod
        && localwb.wbcamMethod == other.localwb.wbcamMethod
        && localwb.nbspot == other.localwb.nbspot
        && localwb.anbspot == other.localwb.anbspot
        && localwb.retrab == other.localwb.retrab
        && localwb.hueref == other.localwb.hueref
        && localwb.chromaref == other.localwb.chromaref
        && localwb.lumaref == other.localwb.lumaref
        && localwb.expwb == other.localwb.expwb
        && localwb.temp == other.localwb.temp
        && localwb.green == other.localwb.green
        && localwb.equal == other.localwb.equal
        && localwb.gamma == other.localwb.gamma
