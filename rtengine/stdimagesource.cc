...
void StdImageSource::getAutoWBMultipliersloc(double &tempitc, double &greenitc, int begx, int begy, int yEn, int xEn, int cx, int cy, int bf_h, int bf_w, double &rm, double &gm, double &bm, const LocWBParams &localr, const WBParams & wbpar, const ColorManagementParams &cmp, const RAWParams &raw)
{
    if (redAWBMul != -1.) {
        rm = redAWBMul;
        gm = greenAWBMul;
    ...
    img->getAutoWBMultipliersloc(tempitc, greenitc, begx, begy, yEn, xEn, cx, cy, bf_h, bf_w, rm, gm, bm, params->localwb, params->wb, params->icm, params->raw);
